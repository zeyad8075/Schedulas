...
public sealed class ActivityEditedEventHandler : INotificationHandler<DomainEventNotification<ActivityEditedEvent>>
{
    private readonly IApplicationDbContext _db;
    private readonly IPushNotificationService _push;

    public ActivityEditedEventHandler(IApplicationDbContext db, IPushNotificationService push)
    {
        _db = db;
        _push = push;
    }

    public async Task Handle(DomainEventNotification<ActivityEditedEvent> notification, CancellationToken cancellationToken)
    {
    ...
    private static (string Title, string Body) BuildArabicMessage(ActivityType type, string activityTitle)
    {
        ...
        var title = $"تحديث على {typeLabel}";
        var body = $"تم إجراء تعديل على \"{activityTitle}\".";
